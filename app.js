...
/**
 * Test route to see if I can make reads to the db
 */
app.get('/test', function(req, res) {
  console.log("this fired");
  new Subscriber({ userId: "blah" }).save(() => {
    console.log("is this saved");
    Subscriber.find().exec((_, messages) => {
...
function receivedMessage(event) {
  ...
  SubscriberController.addToSubscriberList([senderID]);

  Subscriber.find().exec((err, message) => {
  ...
  var messageData = {
    ...
    message: {
      attachment: {
        type: "image",
        payload: {
          url: "https://media.giphy.com/media/l41lZ3fdYxrMFvUWY/giphy.gif"
        }
      }
    }
  };
...
function initializeDb() {
  ...
  mongoose.connect('mongodb://localhost/dondoitdb', { useNewUrlParser: true });
...
/**
 * Starts a timer to get all recipients of the userIds and send it out
 */
function initializeInterval() {
  console.log('initializing interval to send to recipients...');
  setInterval(() => {
    Subscriber.find().exec((_, subscribers) => {
      subscribers.map(s => {
        sendGifMessage(s.userId);
        sendGenericMessage()
      })
    })
  }, 5000);
